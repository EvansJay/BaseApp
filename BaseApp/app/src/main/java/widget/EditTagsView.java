...
public class EditTagsView extends ViewGroup implements OnEditorActionListener,
	...
	public EditTagsView(Context context, AttributeSet attrs) {
		super(context, attrs);
		TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.EditTags);
		try {
			mMaxCount = a.getInteger(R.styleable.EditTags_tag_max_count, DEFAULT_MAX_COUNT);
			mMaxLength = a.getInteger(R.styleable.EditTags_tag_max_length, DEFAULT_MAX_LENGH);
			mHorizontalSpacing = a.getDimensionPixelSize(R.styleable.EditTags_horizontal_spacing, DEFAULT_HORIZONTAL_SPACING);
			mVerticalSpacing = a.getDimensionPixelSize(R.styleable.EditTags_vertical_spacing, DEFAULT_VERTICAL_SPACING);
	...
	private void initView(Context context) {
		...
		mEdtView.setBackgroundResource(R.drawable.shape_attr_item_gray_border);
	...
	public void addTag(CharSequence text) {
		if (text == null) {
			if (mListener != null) {
		...
		if (mTags.toString().contains(text)) {
			if (mListener != null) {
				mListener.onTagError(onTagAddListener.Type.AGAIN);
			}
			return;
		}
	...
	public boolean deleteLastTag() {
		...
		if (mListener != null){
			mListener.onTagDelete(mTags.get(mTags.size() - 1));
		}

...
public class JumpToUtils {
    ...
    public static void JumpToNewApp(String Account, String Password){
        Intent intent = App.getInstance().getPackageManager().getLaunchIntentForPackage("org.birdback.histudents");
        if(intent == null){
            // 这里判断 Intent 为空, 说明应用不存在
            //toast("未安装商家端app,请先安装同学快跑商家端");
            Toast toast = Toast.makeText(App.getInstance(),"",Toast.LENGTH_LONG);
